...
function proxyToBackend(req, res) {
  ...
  req.on('end', () => {
    ...
    proxyReq.on('error', (err) => {
      ...
      console.error('💡 Certifique-se de que o backend Python está rodando na porta 5000');
      console.error('   Execute: python backend.py');
      res.writeHead(502, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify({ 
        status: 'error', 
        message: `Backend Python não está disponível na porta ${BACKEND_PORT}. Certifique-se de que está rodando (python backend.py). Erro: ${err.message}` 
      }));
    });
