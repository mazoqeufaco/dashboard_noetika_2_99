...
async function loadSolutionDescriptions(){
  try {
    console.log(`📥 Carregando ${SOLUTION_DESC}...`);
    const response = await fetch(SOLUTION_DESC, {cache:'no-store'});
    if(!response.ok) {
      console.error(`❌ Erro HTTP ao carregar ${SOLUTION_DESC}: ${response.status} ${response.statusText}`);
      console.error(`   URL tentada: ${SOLUTION_DESC}`);
      throw new Error(`Não foi possível carregar ${SOLUTION_DESC}: ${response.status} ${response.statusText}`);
    }
    solutionDescriptions = await response.json();
    console.log(`✅ ${SOLUTION_DESC} carregado com sucesso`);
    console.log(`   Versão: ${solutionDescriptions?.versao || 'N/A'}`);
    console.log(`   Chaves do JSON:`, solutionDescriptions ? Object.keys(solutionDescriptions) : 'null');
    console.log(`   Itens encontrados: ${solutionDescriptions?.itens?.length || 0}`);
    
    if(!solutionDescriptions) {
      console.error('❌ ERRO: solutionDescriptions é null ou undefined!');
      return null;
    }
    
    if(!solutionDescriptions.itens) {
      console.error('❌ ERRO: solutionDescriptions.itens não existe!');
      console.log('   Chaves disponíveis:', Object.keys(solutionDescriptions));
      return solutionDescriptions; // Retorna mesmo assim para não quebrar
    }
    
    if(solutionDescriptions.itens.length === 0) {
      console.error('⚠️  AVISO: JSON carregado mas array itens está vazio!');
      console.log('   Estrutura do JSON:', Object.keys(solutionDescriptions));
    } else {
      console.log(`   Primeiros 3 itens:`, solutionDescriptions.itens.slice(0, 3).map(i => `${i.id}: ${i.nome_curto || i.nome || 'N/A'}`));
    }
    ...
    solutionDescriptions = null; // Garante que seja null em caso de erro
    return null;
  }
}
...
function showSolutionModal(solutionName, coordStr){
  ...
  if(!solution && coordStr && solutionDescriptions?.itens){
  ...
  if(!solution){
    ...
    console.log('   solutionDescriptions existe?', !!solutionDescriptions);
    console.log('   solutionDescriptions.itens existe?', !!solutionDescriptions?.itens);
    console.log('   Número de itens:', solutionDescriptions?.itens?.length || 0);
    if(solutionDescriptions?.itens && solutionDescriptions.itens.length > 0) {
      console.log('💡 Primeiros 5 itens disponíveis:', solutionDescriptions.itens.slice(0, 5).map(i => `${i.id}: ${i.nome_curto || i.nome || 'N/A'}`));
    } else {
      console.error('❌ Nenhum item encontrado no JSON!');
      console.log('   Estrutura do JSON:', solutionDescriptions ? Object.keys(solutionDescriptions) : 'solutionDescriptions é null');
    }
...
(async () => {
  try {
    ...
    const loaded = await loadSolutionDescriptions();
    if(loaded && loaded.itens && loaded.itens.length > 0) {
      console.log('✅ Descrições de soluções carregadas:', loaded.itens.length, 'itens');
    } else {
      console.error('❌ ERRO: Descrições de soluções NÃO foram carregadas corretamente!');
      console.error('   solutionDescriptions:', solutionDescriptions);
      console.error('   Tentando carregar novamente em 2 segundos...');
      // Tenta carregar novamente após 2 segundos
      setTimeout(async () => {
        const retry = await loadSolutionDescriptions();
        if(retry && retry.itens && retry.itens.length > 0) {
          console.log('✅ Descrições carregadas na segunda tentativa:', retry.itens.length, 'itens');
        } else {
          console.error('❌ Falha ao carregar descrições mesmo na segunda tentativa');
        }
      }, 2000);
    }
